...
void SystemManager::adminLogin()
{
	...
	if (it != va.end())
	{
		...
		pause();
		tmpA.loadRoom(vr);
		tmpA.loadBool(studentFileIsEmpty, teacherFileIsEmpty, adminFileIsEmpty);
		tmpA.loadUsers(vs, vt, va);
		tmpA.toDo();
	}
	else
	{
...
void SystemManager::loadRoom()
{
	...
	while (ifs >> num && ifs >> capacity && ifs >> total)
	{
		...
		for (int i = 0; i < temp.m_Total; i++)
		{
			...
			ifs >> s_Num;
			ifs >> s_Name;
			pair<long, string> t_Pair = make_pair(s_Num, s_Name);
			temp.all_Record.insert(make_pair(t_Day, t_Pair));
		}
		vr.push_back(temp);
		//showRoom(temp);		//测试功能
		num = 0;
...
void SystemManager::showRoom(Room &r)
{
	...
	for (multimap<int, pair<long, string>>::iterator it = r.all_Record.begin(); it != r.all_Record.end(); it++)
	{
